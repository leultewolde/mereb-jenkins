...
## Highlights
- **Config now prefers `.ci/ci.mjc`** – the file contents stay YAML, and legacy `.ci/ci.yml` / `ci.yml` paths still work.
...
1. **Create `.ci/ci.mjc`** using the schema and samples in [`docs/pipeline-config.md`](pipeline-config.md).
