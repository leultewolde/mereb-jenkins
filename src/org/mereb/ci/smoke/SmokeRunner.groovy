package org.mereb.ci.smoke

import static org.mereb.ci.util.PipelineUtils.shellEscape

/**
 * Executes HTTP or script-based smoke tests with retries and timeouts.
 */
class SmokeRunner implements Serializable {

    private final def steps

    SmokeRunner(def steps) {
        this.steps = steps
    }
    ...
    void run(Map args = [:]) {
        ...
        if (!url && !script) {
            steps.error "runSmoke: supply either 'url' or 'script'/'command'"
        }
        ...
        while (attempt < totalAttempts) {
            attempt++
            try {
                steps.echo "runSmoke: ${label} attempt ${attempt}/${totalAttempts}"
                if (timeoutSeconds > 0) {
                ...
                steps.echo "runSmoke: attempt ${attempt} failed: ${err}. Retrying after ${delaySeconds}s"
    ...
    private void runCurl(String url, Map headers) {
        ...
        cmd << url
        steps.sh cmd.collect { shellEscape(it) }.join(' ')
    }
